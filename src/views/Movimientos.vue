<template >
  ...
  async BuscarCodigo(){
    ...
    const respuesta = await this.axios.get(`/api/ConfigForm/BuscarArticulosCodigo?pTipo=${this.idUsuario}&pCodigo=${this.MediCodigo}`)
       ...
       //         this.MediStock = ListaArticulosCodigoLista.stock_restante;
        ...
        //    });
            //////////////////////////////////////
            /////Nuevo Calculo de Medicamento/////
            //////////////////////////////////////
           // this.articulosid = this.articulosid.toString().padStart(6, '0');
  ...
  let IdInst = "";
   ...
   VerDatosAfi(){
     ...
     this.VerDatosText = "Movimientos de articulos";
  ...
  let IdInst = "";
    ...
    data1.forEach(item => { IdInst = item.codigo;   }); })
      .catch(err => { console.log(err); });
        //////////
     this.MostrarSpinner = true; //abrir spinner mientras realiza la solicitud
     const respuesta = await this.axios.get(`/api/ConfigForm/ListaCombo?pTipo=16&pId=${this.idUsuario}`)
       .then((respuesta) => {
