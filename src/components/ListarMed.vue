<template>
 ...
 <div  class="shadow p-3  bg-light text-sm text-left" align-center style="max-width: 1350px; " >
  ...
  <div class="row bg-white" >
    <div class="col-md-6">
  <v-btn @click="ModificaCantidad()"  color="primary"  x-small  prepend-icon="mdi-check-circle" block>Modificar</v-btn> 
  </div>
    <div class="col-md-6">
 ...
 <!-- Alerta de confirmación personalizada -->
     <div v-if="mostrarAlertaGrabar1"  role="alert">
         ...
         <v-btn  prepend-icon="mdi-cancel" color="#FF0000" @click="cerrarGrabar1">   <v-spacer></v-spacer> Cerrar</v-btn>
       </div>
       </div>
           </div>
       </div>
   ...
   <!-- Empieza ventana modificar -->
   <v-dialog
           v-model="VentanaModifcar"
           persistent
           width="500"
 > 
 <v-alert
       ...
       border="top"
     >
     La medicacion se modifico correctamente 
 <!-- Alerta de confirmación personalizada -->
         ...
         <div class="col  p-3 text-center ">
         <v-btn  prepend-icon="mdi-check-circle-outline" color="success" @click="cerrarGrabar1">   <v-spacer></v-spacer> Cerrar</v-btn>
       </div>
       </div>
           </div>
...
<br>
 <div class="row bg-white" >
     ...
     <table class=" table-striped  text-left" style="width: 100%;" ref="table">
       ...
       <tbody>
         ...
         <td>
           <v-btn @click="VerMod(row)" class="bg-success" prepend-icon="mdi-pencil" ></v-btn>
         </td>
         </tr>
 ...
 AlertaMensaje
   ...
   data() {
     return {
       ...
       rows: [],
       VentanaGrabar: false,
       VentanaModifcar: false,
       VentanaModCant: false,
   ...
   methods: {
    ...
    async  ModificaCantidad(){
      
      if (!this.Mod_Cantidad || this.Mod_Cantidad.length == 0) {
    ...
    data1.forEach(item => { IdInst = item.codigo;   }); })
      .catch(err => { console.log(err); });
        //////////
      //Modificar
     await this.axios.put(`/api/ConfigForm/ModificarCantidad?pId=${this.IdInternacion}&pCantidad=${this.Mod_Cantidad}&IdInstitucion=${IdInst}`)
   
   .then(datos => {
  
     
     this.mostrarAlertaGrabar1 = true;
        this.VentanaModifcar = true;
        this.fetchArticulosMed();
 ...
 html2canvas(clonedTable, { scale: 2 }).then(canvas => {
   ...
   document.body.removeChild(tempDiv);
     ...
     CerrarMod(){
      this.VentanaModCant = false;
     }
     ,
     async VerMod(row) {
      //alert("Hola");
      this.VentanaModCant = true;
