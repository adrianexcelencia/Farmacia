 ...
 <!-- Alerta de confirmación personalizada -->
   ...
   <div>
     <v-alert
      ...
      border="top"
    >
     <b>   Listado de medicamentos vencidos  </b> 
...
//-----------------------------
  ...
  pdf.text(Sutitle2, imgWidth / 2, 20, { align: 'center' });
   ...
   pdf.save('Listado-Medicamentos-Vencidos.pdf');
...
exportToExcel() {
      ...
      XLSX.writeFile(workbook, "Listado-Medicamentos-Vencidos.xlsx");
    },
     async  MostrarRemito(){
   ...
   async fetchArticulosMed() {
     ...
     async GenerarInforme() {
      ...
      const respuesta = await this.axios.get(`/api/ConfigForm/ListarVencidos?pTipo=1&pInstitucion=${IdTipo}&pFechaDesde=${fechaDesdeSinGuiones}&pFechaHasta=${fechaHastaSinGuiones}&pdias=0`)
