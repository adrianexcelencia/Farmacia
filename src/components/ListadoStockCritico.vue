<template>
  ...
  <br>
   ...
   html2canvas(clonedTable, { scale: 2 }).then(canvas => {
     ...
     const title = "Listado de Stock Critico"; // Reemplaza con tu título
     pdf.setFontSize(10);
     ...
     pdf.save('InformeStockCritico.pdf');
     ...
     document.body.removeChild(tempDiv);
       ...
       XLSX.writeFile(workbook, "InformeStockCritico.xlsx");
     },
       async  MostrarDatos(){ 
  ...
  data.forEach(item => { IdTipo = item.codigo;   }); })
    .catch(err => { console.log(err); });
        ...
        const respuesta = await this.axios.get(`/api/ConfigForm/ListaArticulosStockCritico?pTipo=${IdTipo}`)
           .then((respuesta) => {
             //this.ListaFormulariosArticulos = respuesta.data.lista
             this.rows = respuesta.data.lista;
